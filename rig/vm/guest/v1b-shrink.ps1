# V1b step 2 (guest side, ELEVATED): shrink C: to free space for the alongside
# install, and record everything the gate needs from the Windows side.
#   powershell -ExecutionPolicy Bypass -File \\10.0.2.4\qemu\rig\vm\guest\v1b-shrink.ps1
# Windows PowerShell 5.1 only. Writes artifacts/v1b/shrink.json + shrink.txt on
# the host share, and registers the v1b-mark logon task.
$ErrorActionPreference = 'Continue'
$share = '\\10.0.2.4\qemu'
$out = Join-Path $share 'rig\vm\artifacts\v1b'
...
Log "=== V1b shrink $($r.timestamp) elevated=$($r.elevated) ==="
...
# --- boot-marker task + OEMDRV visibility ------------------------------------
New-Item -ItemType Directory -Force -Path 'C:\upgrade_\rig' | Out-Null
Copy-Item (Join-Path $share 'rig\vm\guest\v1b-mark.ps1') 'C:\upgrade_\rig\v1b-mark.ps1' -Force
...
Log "=== shrink done; shutdown when ready (shutdown /s /t 0) ==="
